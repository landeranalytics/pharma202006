...
params:
    className: "ML in R"
    repoName: "pharma202006"
    repoOwner: "landeranalytics"
    organizer: "[Lander Analytics](https://www.landeranalytics.com)"
    packages: !r c('glmnet', 'xgboost', 'rsample', 'recipes', 'coefplot', 'DiagrammeR', 'ranger', 'randomForest', 'partykit', 'tune', 'parsnip', 'piggyback')
    rver: 4.0.0
---
...
Then open a browser and visit `localhost:8787`, login with username `rstudio` and password `password`.
...
If you are running R `r params$rver`, then to install the necessary packages run the following.

```{r restore,eval=FALSE}
renv::restore()
```

If you have a different version of R, then running the following line will work.

```{r install-packages-text,echo=FALSE,comment=NA}
...
If you are still having trouble setting up the project and getting everything installed, sign up for an [RStudio.cloud](https://rstudio.cloud/spaces/76768/join?access_code=nSoEhO6etv%2BHkVl%2B1I%2FmK2YSOPTmFQhDgMIcEjPI) account.^[The compute instance has limited processing and memory so performance on some topics may be suboptimal.]
